namespace $ {
	export function $mol_view_tree_ts_value(
		...
		if (type === '*') return this.$mol_view_tree_ts_multiple_dictionary(prop_parts, context)

		if (type[0] === '/') return this.$mol_view_tree_ts_multiple_array(prop_parts, context)
		...
		if ($mol_view_tree_ts_simple_detect(operator)) return this.$mol_view_tree_ts_simple(operator, prop_parts.name, context)
