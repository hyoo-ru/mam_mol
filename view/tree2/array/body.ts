namespace $ {
	 ...
	 * ```
	 */
	export function $mol_view_tree2_array_body(
		...
		for (const opt of kids) {
			...
			else if (type === '<=') value = this.$mol_view_tree2_bind_left(opt, context)
			else if (type === '*') value = this.$mol_view_tree2_dictionary(opt, context)
			else if (type[0] === '/') value = this.$mol_view_tree2_array(opt, context)
			else value = this.$mol_view_tree2_literal(opt)
