...
namespace $.$$ {
	...
	export class $mol_hyperhive extends $.$mol_hyperhive {
		...
		@ $mol_mem
		authentificated( next? : boolean , force? : $mol_mem_force ) : boolean {
			...
			hhfw.Auth(
				this.login() ,
				this.password() ,
				( message : any )=> this.authentificated( true , $mol_mem_force_cache ) ,
				( message : any )=> this.authentificated( new Error( message ) as any , $mol_mem_force_cache ) ,
			)
		...
		@ $mol_mem
		resources( next? : any , force? : $mol_mem_force ) : boolean {
			this.authentificated().valueOf()

			hhfw.GetResource(
				( message : any )=> this.resources( JSON.parse( message ).data , $mol_mem_force_cache ) ,
				( message : any )=> this.resources( new Error( `${ message }` ) as any , $mol_mem_force_cache ) ,
			)
		...
		@ $mol_mem_key
		data< Value >( table : string , next? : any , force? : $mol_mem_force ) : Value {
			...
			const handleError = ( message : string ) => {
				...
				this.data( table , error , $mol_mem_force_cache )
			}
			...
			if( next === void 0 ) {
					...
					( result : any ) => {
						setTimeout( ()=> {
							hhfw.Query(
								...
								( resp : string )=> this.data( table , JSON.parse( resp ).data || [] , $mol_mem_force_cache ) ,
								handleError ,
							)
						})
					} ,
				...
				hhfw.Post(
					...
					table ,
					JSON.stringify( next ) ,
					( resp : any )=> this.data( table , void 0 , $mol_mem_force_cache ) ,
					handleError
				)
			}
