namespace $ {
	...
	$mol_test({
		...
		'every path×method becomes export const with method/route/params/query/body/result'() {
			...
			$mol_assert_ok( out.startsWith( 'namespace \x24.\x24x' ) )
			$mol_assert_ok( /export const listPets = \{/.test( out ) )
			$mol_assert_ok( /export const getPet = \{/.test( out ) )
			$mol_assert_ok( /export const updatePet = \{/.test( out ) )
			$mol_assert_ok( /method: "GET"/.test( out ) )
			$mol_assert_ok( /method: "POST"/.test( out ) )
		},
		...
		'path params typed via operations[opId][parameters][path] when there are any'() {
			...
			$mol_assert_ok( out.includes( `operations[ "getPet" ][ 'parameters' ][ 'path' ]` ) )
		},
		...
		'result typed via operations[opId][responses][200][application/json] infer'() {
			...
			$mol_assert_ok( out.includes( `operations[ "listPets" ][ 'responses' ][ 200 ]` ) )
			$mol_assert_ok( out.includes( `infer R` ) )
		},
		...
		'success code falls back to 201 when no 2xx earlier'() {
			...
			$mol_assert_ok( out.includes( `operations[ "updatePet" ][ 'responses' ][ 201 ]` ) )
		},
		...
		'body typed via operations[opId][requestBody] infer'() {
			...
			$mol_assert_ok( /body: \{\} as \( operations\[ "updatePet" \]\[ 'requestBody' \]/.test( out ) )
		},
		...
		'no path params → params typed as undefined'() {
			...
			$mol_assert_ok( /export const listPets = \{[\s\S]*?params: undefined as undefined/.test( out ) )
		},
		...
		'operation satisfies $mol_openapi_operation generic'() {
			...
			$mol_assert_ok( out.includes( 'satisfies \x24mol_openapi_operation<' ) )
		},
		...
		'\x24defs declaration from openapi-typescript is stripped to avoid fake /defs dep'() {
			...
			$mol_assert_ok( !out.includes( '\x24defs' ) )
			$mol_assert_ok( out.includes( 'webhooks' ) )
			$mol_assert_ok( out.includes( 'paths' ) )
		},
		...
		'unique operation name when operationId absent (derived from method_route)'() {
			...
			$mol_assert_ok( /export const get_a = \{/.test( out ) )
			$mol_assert_ok( /export const get_a_b = \{/.test( out ) )
		},
